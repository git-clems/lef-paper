...
def LocalSear(Y):
    ...
    for _ in range(200):
        for i in I:
            if any(_Y[:,i] == 1):
                _Y[:,i] = np.zeros((NB_T))
                new_t = np.random.randint(0,NB_I)
                _Y[new_t,i] = 1
                
        _Z = Objective(_Y, _XO, _XC, _Buy, _Sold)
        if _Z < Z:
            XO, XC, Buy, Sold = _Y, _XO, _XC, _Buy, _Sold
...
Y_opt, XO_opt, XC_opt, Buy_opt, Sold_opt, Z_opt, obj_history = VNS(ntr_max=500,k_max=3, time_limit=10000)
