...
r = np.array([0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35])

    # file.write(f'cost = [')
    # file.write(f'ox = [')
    # file.write(f'no_policy_cost, no_policy_emi = ')

for _ in range(len(r)):
    with open('Sensitivity_analysis/plan.txt', 'a') as file:
        file.write(f'*** u = {100*r[_]}% ***\n')
    ...
    maintain_cost = quicksum(r[_] * V * b[i] * Y[tp, i] for i in I for t in T for tp in range(t + 1))
    ...
    if model.status == GRB.OPTIMAL:
        ...
        # First-stage cost
        install_cost = sum(V * b[i] * Y_val[t,i] for t in T for i in I) + sum(r[_] * V * V*  b[i] * Y_val[tp,i]for i in I for t in T for tp in range(t + 1))
        # Second-stage expected cost
        ...
        with open('Sensitivity_analysis/plan.txt', 'a') as file:
            ...
            # file.write(f'{np.round(model.ObjVal,2)}, {np.round(emission,2)} ')
            file.write(f'{np.transpose(Y_val)}\n')
            file.write(f'{sum(b[i]*Y_val[t,i] for t in T for i in I)}\n\n')
