...
with open("Result_computational/avg_result.csv","w") as file:
    ...
    for _ in range(gurobi_data.shape[0]):
        file.write(f'{gurobi_data.index[_]} \t& {np.round(gurobi_cost[_],2)} \t& {np.round(vns_cost[_],2)} \t& {np.round(sa_cost[_],2)} \t& {np.round(vns_ql_cost[_],2)} \t&  {gurobi_gap[_]} \t& {np.round(100*(vns_cost[_] - gurobi_cost[_])/gurobi_cost[_],2)} \t& {np.round(100*(sa_cost[_] - gurobi_cost[_])/gurobi_cost[_],2)} \t& {np.round(100*(vns_ql_cost[_] - gurobi_cost[_])/gurobi_cost[_],2)} \\\ \n')
...
plt.ylabel("CPU time (Second)", fontsize = 20)
